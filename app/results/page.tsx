...
import { downloadFile, getJob, getSessionState, type JobResult } from "@/lib/api"
...
function ResultsContent() {
  ...
  const [resolvedJobId, setResolvedJobId] = useState(jobIdParam)
  ...
  useEffect(() => {
    ...
    async function resolveJobId() {
      ...
      if (!active) return
      setResolvedJobId(session?.lastJobId || "")
    }
  ...
  useEffect(() => {
    ...
    async function fetchJob() {
      setIsLoading(true)
      try {
        const result = await getJob(resolvedJobId)
      ...
      <div className="container mx-auto max-w-7xl px-4 py-8">
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="mt-4 text-lg font-medium">Aucun scenario disponible</p>
            <p className="text-muted-foreground">
              Lancez une allocation pour afficher les resultats.
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold">Resultats de l{"'"}Allocation</h1>
